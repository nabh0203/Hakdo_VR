...
public class BleedBehavior1 : MonoBehaviour
{
    ...
    public void Update()
    {
        ...
        // Raycasting from the camera's position forward
        RaycastHit hit;
        if (Physics.Raycast(Camera.main.transform.position, Camera.main.transform.forward, out hit))
        {
            // If the raycast hits an object with a specific tag (for example "Interactable")
            if (hit.collider.tag == "Anemy")
            {
                // Enable the texture effect
                BloodAmount = 1; // Or any desired value between 0 and 1
            }
        }
